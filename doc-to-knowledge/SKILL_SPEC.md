...
读取飞书文档（视频转录稿），以 **中小客 (SMB)** 视角，用 AI 从中拆解出尽可能多的**独立、可发布**的知识点，并将结构化结果写入飞书多维表格，供人工审核后流入下游。
...
| `核心内容` | 文本 | ✅ | 知识点的完整描述（200-500 字，面向中小客视角重写） |
| `关键要点` | 文本 | ✅ | 3-5 个 bullet point，每条一句话 |
| `适用场景` | 文本 | ✅ | 这个知识点最适用于什么业务场景 |
| `痛点标签` | 多选 | ✅ | 中小客痛点标签，如 `预算有限` `缺乏经验` `人手不足` `获客困难` |
| `难度等级` | 单选 | ✅ | `入门` / `进阶` / `高级` |
